...
func testFileLevelDict(t *testing.T, fn string, level int, d string) {
	...
	golden, err := os.Open(filepath.Join(runtime.GOROOT(), "src", "compress", "zlib", fn))
	if err != nil {
...
func testFileLevelDictReset(t *testing.T, fn string, level int, dict []byte) {
	...
	var err error
	if fn != "" {
		b0, err = os.ReadFile(filepath.Join(runtime.GOROOT(), "src", "compress", "zlib", fn))
		if err != nil {
...
func TestWriterBig(t *testing.T) {
	for i, fn := range filenames {
		...
		for level := BestSpeed; level <= BestCompression; level++ {
			...
			if level >= 1 && testing.Short() && testenvBuilder() == "" {
				break
			}
		}
		if i == 0 && testing.Short() && testenvBuilder() == "" {
			break
		}
	}
}
